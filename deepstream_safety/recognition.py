"""Standalone recognition lifecycle copied from the Frigate contracts.
...
def _jaro_winkler(left: str, right: str) -> float:
    ...
    transpositions = sum(a != b for a, b in zip(left_order, right_order)) / 2
    jaro = (
        matches / len(left)
        + matches / len(right)
...
class RecognitionCore:
    """Track-scoped cadence and aggregation matching Frigate's recognition core."""
