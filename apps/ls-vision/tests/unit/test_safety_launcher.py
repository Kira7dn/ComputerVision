...
APP_ROOT = ROOT / "apps" / "ls-vision"


def test_jetson_dev_launcher_uses_source_sync_and_vite_hmr() -> None:
    launcher = (APP_ROOT / "deploy" / "powershell" / "deploy-jetson-dev.ps1").read_text(encoding="utf-8")
...
def test_cpu_analysis_branch_has_a_terminal_sink() -> None:
    pipeline = (APP_ROOT / "src" / "ls_vision" / "adapters" / "deepstream" / "runtime.py").read_text(
        encoding="utf-8"
    )
...
def test_mediamtx_reader_exposes_stats_and_dashboard_api_exposes_webrtc() -> None:
    ...
    server = (APP_ROOT / "src" / "ls_vision" / "interfaces" / "dashboard_api.py").read_text(
        encoding="utf-8"
    )
...
def test_live_output_follows_rtsp_sample_contract() -> None:
    ...
    launcher = (APP_ROOT / "deploy" / "powershell" / "deploy-jetson.ps1").read_text(
        encoding="utf-8"
    )
...
def test_jetson_dev_service_is_isolated_from_production() -> None:
    service = (APP_ROOT / "deploy" / "systemd" / "ls-vision-dev.service").read_text(
        encoding="utf-8"
    )
    ...
    launcher = (APP_ROOT / "deploy" / "powershell" / "deploy-jetson-dev.ps1").read_text(
        encoding="utf-8"
    )
...
def test_vision_ingress_has_no_tbox_service_dependency() -> None:
    ingress = (APP_ROOT / "deploy" / "systemd" / "ls-vision-ingress.service").read_text(
        encoding="utf-8"
    )
...
def test_config_routes_functions_per_camera() -> None:
    config_path = APP_ROOT / "config" / "production.yaml"
...
def test_config_is_valid_yaml_and_has_stable_evidence_contract() -> None:
    config_path = APP_ROOT / "config" / "dev.yaml"
...
def test_fire_smoke_canary_model_can_be_overridden_for_one_camera() -> None:
    raw = load_raw_config(APP_ROOT / "config" / "production.yaml")
...
def test_runtime_can_disable_notifications_for_acceptance(monkeypatch) -> None:
    ...
    resolved = resolve_camera_config(
        load_raw_config(APP_ROOT / "config" / "production.yaml"),
        "DMS",
    )
...
def test_event_feed_collapses_lifecycle_and_recognition_starts_on_exact_frame() -> None:
    dashboard_server = (APP_ROOT / "src" / "ls_vision" / "interfaces" / "dashboard_api.py").read_text(
        encoding="utf-8"
    )
    pipeline = (APP_ROOT / "src" / "ls_vision" / "adapters" / "deepstream" / "runtime.py").read_text(
        encoding="utf-8"
    )
    notifications = (APP_ROOT / "src" / "ls_vision" / "adapters" / "notification" / "service.py").read_text(
        encoding="utf-8"
    )
...
def test_unknown_recognition_event_uses_best_frame_after_track_end() -> None:
    ...
    face_engine = (APP_ROOT / "src" / "ls_vision" / "adapters" / "models" / "face_engine.py").read_text(
        encoding="utf-8"
    )
