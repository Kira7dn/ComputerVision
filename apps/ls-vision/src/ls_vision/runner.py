...
from ls_vision.adapters.persistence.evidence_repository import run_directory, write_manifest
from ls_vision.bootstrap.config import camera_ids, load_raw_config, validate_config
from ls_vision.bootstrap.lifecycle import install_shutdown_handlers
...
def main() -> int:
    ...
    def command_for(camera_id: str) -> list[str]:
        ...
        return [
            sys.executable,
            "-m",
            "ls_vision.application.camera_runtime",
            "--config",
            str(args.config),
            "--camera-id",
            camera_id,
    ...
    install_shutdown_handlers(lambda: stop_workers())
    try:
