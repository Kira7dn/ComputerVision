...
from ls_vision.bootstrap.paths import RuntimePaths
...
def _load_raw_config(path: Path) -> dict[str, Any]:
    ...
    if "extends" in data:
        raise ValueError("config inheritance is not supported; profiles must be standalone")
    return data
...
def _apply_runtime_environment_overrides(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if environment.get("CAMERA_RUNTIME_ROOT"):
        paths = RuntimePaths.from_environment()
        runtime["status_directory"] = str(paths.status)
        runtime["state_directory"] = str(paths.state)
        runtime["queue_directory"] = str(paths.queue)
        runtime["log_directory"] = str(paths.logs)
        evidence["directory"] = str(paths.evidence)
    return config
...
def validate_config(config: dict[str, Any], path: Path | None = None) -> dict[str, Any]:
    ...
    evidence_dir = Path(str((config.get("evidence", {}) or {}).get("directory", ".tmp/ls-vision")))
...
def camera_ids(config: dict[str, Any]) -> list[str]:
    ...
    if cameras is None:
        raise ValueError("platform config must define cameras")
...
def _dotenv_values(path: Path) -> dict[str, str]:
    """Read the small local environment file used by deployment tooling."""
    if not path.is_file():
        return {}
    values: dict[str, str] = {}
...
def _normalize_camera_analysis(
    ...
    """Apply the public per-camera analysis schema to model sections."""
...
def resolve_camera_config(config: dict[str, Any], camera_id: str | None = None) -> dict[str, Any]:
    """Resolve one camera from the canonical multi-camera configuration."""
    ids = camera_ids(config)
    if camera_id is None:
        ...
        camera_id = ids[0]
    camera = _camera_by_id(config, camera_id)
