...
LS-Vision là runtime DeepStream canonical dưới `apps/src` với các module trực tiếp,
không có namespace bọc ngoài.

```text
service
  ├─ interfaces.dashboard_api
  ├─ runner
  │   └─ application.camera_runtime per non-media-only camera
  └─ interfaces.mock_media_server when configured
```
