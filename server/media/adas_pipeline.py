...
class AdasPipeline:
    def __init__(self, channel, event_log, output_dir):
        ...
        self.model_path = Path(os.environ.get(
            'ADAS_MODEL_PATH', self.output_dir.parent.parent / 'models' / 'yolov8n.engine'
        ))
        self.mediamtx_rtsp = os.environ.get(
            'MEDIAMTX_PUBLISH_URL', f'rtsp://127.0.0.1:8554/adas-ch{channel}'
        )
    ...
    def _validate_runtime(self):
        if not shutil.which('ffmpeg'):
            raise RuntimeError('ffmpeg is required for ADAS decode/publish')
        ...
        try:
            ...
            raise RuntimeError(f'ADAS Python dependency missing: {exc}') from exc
        self.status = 'warming_up'
        self.model = YOLO(str(self.model_path), task='detect')
        # Warm-up is part of health semantics, never first-request work.
        import numpy as np
        self.model.predict(
            np.zeros((FRAME_HEIGHT, FRAME_WIDTH, 3), dtype=np.uint8),
            imgsz=(FRAME_HEIGHT, FRAME_WIDTH), batch=1, verbose=False,
        )
    ...
    def _inference_worker(self):
        ...
        while not self.stop_event.is_set():
            ...
            if item is None:
                continue
            _sequence, frame, received_at, source_pts_ms, source_capture_at = item
            ...
            try:
                result = self.model.predict(
                    frame, imgsz=(FRAME_HEIGHT, FRAME_WIDTH), batch=1,
                    conf=0.25, iou=0.45, classes=sorted(TARGET_CLASSES),
                    verbose=False,
                )[0]
                ...
                for box in result.boxes:
                    ...
                    if self.warning_enabled and rx1 <= cx <= rx2 and ry1 <= cy <= ry2:
                        if warning_candidate is None or confidence > warning_candidate['confidence']:
                            warning_candidate = {
                    ...
                    cv2.putText(frame, f'{name} {confidence:.2f}', (x1, max(16, y1 - 5)),
                ...
                cv2.putText(frame, f'decode->detect {latency_ms:.0f} ms', (8, 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 1)
                cv2.putText(frame, f'dropped {self.frames_dropped_stale}', (8, 42),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 1)
                received_wall = datetime.fromtimestamp(
                    received_at + self.wall_clock_offset
                ).strftime('%H:%M:%S.%f')[:-3]
                ts_label = f'TS recv {received_wall}'
                if source_pts_ms is not None:
                    ts_label += f' PTS {source_pts_ms} ms'
                (label_width, _label_height), _baseline = cv2.getTextSize(
                    ts_label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1
                )
                cv2.putText(frame, ts_label, (FRAME_WIDTH - label_width - 8, 64),
