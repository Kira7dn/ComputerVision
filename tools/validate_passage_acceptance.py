"""Two-camera passage acceptance entrypoint."""
...
def assign_records(
    ...
    for record in records:
        ...
        candidates = [
            passage
            for passage in passages_by_camera[camera]
            if float(passage["start_s"]) - 0.3
            <= source_time
        ...
        if len(candidates) > 1 and box is not None:
            scored = [
                (bbox_iou(box, [float(v) for v in p["bbox"]]), p)
                for p in candidates
                if p.get("bbox")
            ]
    ...
    return (
        min(
            ...
            key=lambda value: float(
                value.get("trace_time", value.get("frame_time", 0))
            ),
        )
        if found
        else None
...
def correlation_mismatches(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for record in records:
        if record.get("stage") not in {
            ...
            continue
        required = ("candidate_id", "frame_time", "quality_score", "quality_components")
...
def save_mismatch_evidence(
    ...
    for row in rows:
        ...
        capture.set(
            ...
            ((float(passage["start_s"]) + float(passage["end_s"])) / 2) * 1000,
        )
        ok, frame = capture.read()
        capture.release()
        if not ok or frame is None:
...
def main() -> int:
    ...
    summary: dict[str, Any] = {
        "schema_version": 2,
        "profile": "two-camera-passage-acceptance",
        "accepted": False,
        "gates": {},
        "timing": {},
    }
    ...
    try:
        ...
        if args.disable_quality:
            for camera in value.get("cameras", {}).values():
                camera.pop("quality", None)
        ...
        if args.prepare_only:
            summary["gates"] = {
                ...
                "composites_under_15": replay_duration(output / "face-replay.mp4") <= 15
                and replay_duration(output / "lpr-replay.mp4") <= 15,
            }
        ...
        (
            ...
            first_attempt,
            embeddings,
        ) = face_results(records, face_passages, anchors[CAMERAS["face"]])
        ...
        if args.control_summary is not None:
            control = json.loads(args.control_summary.read_text(encoding="utf-8"))
            control_resources = control.get("runtime", {}).get("resources", {})
            resources["control"] = {
                "summary": str(args.control_summary),
                "ram_max_bytes": control_resources.get("ram_max_bytes"),
                "skipped_fps_max": control_resources.get("skipped_fps_max", {}),
            }
        summary.update(
            {
                "face": face,
        ...
        failed_rows = [row for row in face["passages"] if not row["correct"]] + [
            row
            for row in lpr["passages"]
            if not row["detected"] or row["exact"] is False
        ]
        failed_rows += [
            {
                "passage_id": item.get("passage_id"),
                "mismatch_reason": "false_passage",
                "record": item,
            }
            for item in face_false + lpr_false
        ]
        summary["mismatch_evidence"] = save_mismatch_evidence(
            output, manifest, failed_rows
        )
        evidence_ok = all(
            Path(item["path"]).is_file()
            and sha256(Path(item["path"])) == item["sha256"]
            for item in summary["mismatch_evidence"]
        )
        ...
        if control is not None:
            control_resources = control.get("runtime", {}).get("resources", {})
            control_skipped = control_resources.get("skipped_fps_max", {})
            gates["skipped_fps_vs_control"] = skipped_fps_within_control(
                skipped_fps, control_skipped
            )
            control_ram = control_resources.get("ram_max_bytes")
            gates["ram_vs_control"] = (
                control_ram is not None
                and resources["ram_max_bytes"] is not None
                and resources["ram_max_bytes"] <= int(control_ram) + 128 * 1024**2
            )
        summary["gates"] = gates
    except Exception as exc:
