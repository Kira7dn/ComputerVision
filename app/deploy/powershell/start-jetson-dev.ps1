...
foreach ($port in @($VitePort, 18080, 8888, 8889)) {
...
try {
    ...
    $script:tunnelProcess = Start-Process -FilePath 'ssh' -ArgumentList @(
    ...
    while ($true) {
        ...
        foreach ($process in @($script:syncProcess, $script:tunnelProcess, $script:viteProcess)) {
    ...
    foreach ($process in @($script:viteProcess, $script:tunnelProcess, $script:syncProcess)) {
