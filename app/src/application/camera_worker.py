...
dependency on the legacy tracker runtime or its configuration.
"""
...
class SafetyPipeline:
    def __init__(self, config: dict[str, Any], config_path: Path, run_id: str) -> None:
        ...
        self.status_dir = Path(str(runtime.get("status_directory", "/opt/camera-safety/status")))
...
def _run_e2e_mock_worker(config_path: Path, camera_id: str, run_id: str, worker_epoch: str) -> int:
    ...
    status_dir = Path(str((config.get("runtime", {}) or {}).get("status_directory", "/opt/camera-safety/status")))
    ...
    try:
        while not stopping and not hls_ready:
            if publisher.poll() is not None:
                ...
                return 1
            try:
                with urlopen(hls_url, timeout=2.0) as response:  # noqa: S310 - internal Compose URL
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path(os.environ.get("CAMERA_CONFIG", "/opt/camera-safety/config/production.yaml")),
    )
    ...
    if str(raw_config.get("profile", "")).lower() == "e2e":
        raise SystemExit(
            _run_e2e_mock_worker(
                args.config,
                args.camera_id or "camera",
                run_id,
