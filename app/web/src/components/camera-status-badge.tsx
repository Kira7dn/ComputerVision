...
export function CameraStatusBadge({ camera, state }: CameraStatusBadgeProps) {
  ...
  const label = state.live ? (state.transport === 'webrtc' ? 'LIVE' : 'HLS') : state.error || !camera.running ? 'OFFLINE' : 'CONNECTING'
  const Icon = state.live ? (state.transport === 'webrtc' ? CheckCircle2 : Radio) : state.error || !camera.running ? WifiOff : CircleDashed
