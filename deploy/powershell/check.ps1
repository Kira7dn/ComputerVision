...
& $python -m pytest -c (Join-Path $appRoot 'pyproject.toml') $testsRoot -q
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $python -m ruff check (Join-Path $appRoot 'src') $testsRoot
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

& $python -m compileall -q (Join-Path $appRoot 'src') $testsRoot
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
...
$forbidden = & rg -n -i `
    ...
    $legacyPattern `
    $appRoot $testsRoot (Join-Path $cameraRoot 'config') (Join-Path $cameraRoot 'deploy') `
