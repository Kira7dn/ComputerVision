...
$appPath = Join-Path $cameraPath 'apps'
...
$servicePath = Join-Path $deployPath "systemd\$serviceName"
$mediaServicePath = Join-Path $deployPath 'systemd\mediamtx.service'
$mdnsServicePath = Join-Path $deployPath 'systemd\ls-vision-mdns.service'
$ingressServicePath = Join-Path $deployPath 'systemd\ls-vision-ingress.service'
...
if ($Action -eq 'start') {
    ...
    try {
        ...
        $syncScript = Join-Path $deployPath 'dev\jetson_sync.py'
...
try {
    ...
    Write-Step "Packaging apps $DeploymentProfile source"
    ...
    $configPath = if ($DeploymentProfile -eq 'production') {
        Join-Path $configRoot 'production.yaml'
    } else {
        Join-Path $configRoot 'dev.yaml'
    }
    ...
    if ($LASTEXITCODE -ne 0) { throw 'Unable to copy apps archive' }
