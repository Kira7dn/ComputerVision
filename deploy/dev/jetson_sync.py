"""Synchronize apps changes to a native Jetson development runtime.
...
Secrets outside apps are intentionally never synchronized.
"""
...
def snapshot(local_root: Path, *, remote_prefix: str = "") -> dict[str, tuple[int, int]]:
    ...
    for directory, names, files in os.walk(local_root):
        ...
        for name in files:
            ...
            try:
                ...
                continue
            relative = remote_prefix + candidate.relative_to(local_root).as_posix()
...
def send_files(target: str, workspace_root: Path, remote_app: str, paths: list[str]) -> None:
    if not paths:
        return
        ...
        with tarfile.open(fileobj=ssh.stdin, mode="w|gz") as archive:
            for relative in paths:
                source = (
                    ...
                    else workspace_root / "apps" / Path(relative)
                )
                if source.is_file():
...
def sync(
    ...
    send_files(target, workspace_root, remote_app, changed)
...
def main() -> int:
    ...
    if previous is None:
        ...
        sync(args.jetson, workspace_root, args.remote_app, previous, current)
        save_state(state_file, current)
    if args.once:
        return 0

    print(f"watching {app_root} and {config_root} -> {args.jetson}:{args.remote_app}", flush=True)
    previous = current
    try:
        while True:
